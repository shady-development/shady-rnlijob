...
function TakeOutVehicle(vehicleInfo)
    ...
    QBCore.Functions.SpawnVehicle(vehicleInfo, function(veh)
        SetVehicleNumberPlateText(veh, "shady-rnlijob"..tostring(math.random(1000, 9999)))
...
function MenuGarage()
    ...
    for veh, label in pairs(authorizedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "",
            params = {
                event = "shady-rnlijob:client:TakeOutVehicle",
                args = {
                    vehicle = veh
                }
            }
...
RegisterNetEvent('shady-rnlijob:client:TakeOutVehicle', function(data)
...
RegisterNetEvent('QBCore:Client:OnJobUpdate', function(JobInfo)
    PlayerJob = JobInfo
    TriggerServerEvent("ambulance:server:Setshady-rnlijob")
end)
...
CreateThread(function()
    ...
    while true do
        local sleep = 2000
        if LocalPlayer.state.isLoggedIn and PlayerJob.name == "shady-rnlijob" then
                    ...
                    if onDuty then
                        ...
                        if #(pos - v) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", "shady-rnlijobstash_"..QBCore.Functions.GetPlayerData().citizenid)
                                TriggerEvent("inventory:client:SetCurrentStash", "shady-rnlijobstash_"..QBCore.Functions.GetPlayerData().citizenid)
                            end
...
CreateThread(function()
    while true do
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if PlayerJob.name =="shady-shady-shady-shady-shady-shady-rnlijob" then
